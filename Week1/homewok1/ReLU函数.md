...
# 一、针对「死亡ReLU」的负半轴激活优化（线性负半轴）

这类变体为ReLU负半轴赋予非零梯度，从根本上解决神经元永久失活问题，核心是线性扩展负半轴，计算复杂度与ReLU接近。
...
# 二、兼顾「零中心化+鲁棒性」的非线性负半轴优化

线性负半轴变体未解决输出非零中心化，这类变体将负半轴设计为非线性函数，让输出均值接近0，同时提升对噪声的鲁棒性，代价是计算复杂度略有增加。
...
# 三、融合「概率/门控」的平滑非线性优化（大模型标配）

这类变体突破了ReLU的分段线性特性，设计为全局平滑的非线性函数，融合概率思想/自门控机制，让梯度更连续、泛化性更强，是Transformer大模型、高精度CV任务的主流选择，代价是计算复杂度进一步提升。
...
这类变体为移动端/边缘设备设计，通过截断激活值实现数值稳定性，适配网络量化（将浮点型权重/激活值转为整型），兼顾性能和硬件推理速度，是轻量化CNN的主流选择。
